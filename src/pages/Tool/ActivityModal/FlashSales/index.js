...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'flashsales_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addFlashSales' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  getEditData = ( id ) => {
    ...
    if( id ){
      dispatch( {
        type: 'activity/getFlashSaleSpecsList',
        payload: {
          id,
        },
        callFunc: ( result ) => {
  ...
  handleSubmit = (  ) => {
    ...
    if ( basiceData && styleData && listData ){
      ...
      if( data.flashSaleSpecs.length === 0 ) {
        this.setState( {
          loading: false
        } )
        return message.error( '至少配置一个秒杀产品' )
      }
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activitySeckillUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activitySeckillUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='秒杀预览'
                frameBorder={0}
                src={`${serviceObj.activitySeckillUrl}?preview=true`}
                // src="http://192.168.35.114:8080/#/?preview=true"

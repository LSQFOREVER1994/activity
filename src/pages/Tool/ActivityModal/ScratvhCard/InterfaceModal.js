...
const ImgObj ={
  head:`${activityTemplateObj}guaguaka/introductionImg.jpg`,
  background:`${activityTemplateObj}guaguaka/backgroundImg.png`,
  button:`${activityTemplateObj}guaguaka/buttonImg.png`,
  ruleButton:`${activityTemplateObj}guaguaka/ruleButtonImg.png`,
  bottom:`${activityTemplateObj}guaguaka/bottomImg.jpg`, // 取消默认值
}
...
class StyleModal extends PureComponent {
  ...
  constructor( props ) {
    ...
    this.state = {
      ...
      backgroundColor:this.props.data.backgroundColor ?  ( this.props.data.backgroundColor.indexOf( '#' ) !=-1 ? this.props.data.backgroundColor : `#${this.props.data.backgroundColor}` ) : '#f5641b',
  ...
  styleHandleSubmit = () => {
    ...
    form.validateFields( ( err, fieldsValue ) => {
      ...
      Data = id ? Object.assign( 
        fieldsValue,
        { id, buttonColor, backgroundColor }
        ) :
        Object.assign( 
          fieldsValue,
          ...
          <Form layout='horizontal' className={`${styles.formHeight} ${styles.settingImg}`} onSubmit={this.styleHandleSubmit}>
            <Tabs defaultActiveKey="1" onChange={this.changeTab}>
                ...
                >
                  <FormItem>
                    {getFieldDecorator( 'introductionImg', {
                        rules: [{ required: true, message: '请上传活动banner图片' }],
              ...
              <TabPane tab={<TabName name='背景图' errorFormList={errorFormList} requiredList={['backgroundColor', 'backgroundImg']} isActive={parseInt( currTab, 10 ) === 2}  />} key="2">
                ...
                <Content
                  sizeText='560px*306px'
                  imgBoxStyle={{ height: 540, width: 250 }}
                >
                  <FormItem>
                    {getFieldDecorator( 'buttonImg', {
                        rules: [{ required: true, message: '请上传刮奖区覆盖图片' }],
                ...
                >
                  <FormItem>
                    {getFieldDecorator( 'bottomImg', {
                      initialValue: data.bottomImg !== undefined ? data.bottomImg : ImgObj.bottom,
                    } )( <UploadImg onChange={this.imgChang} /> )}
                  </FormItem>
                </Content>
              </TabPane>
...
const Reminder = ( { type, msg = '', style={} } ) =>{
  ...
  switch ( type ) {
    ...
    case 'backgroundColor': text = '设置页面背景色，填充页面除Banner和刮刮卡的背景区域'; break;

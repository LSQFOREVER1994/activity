...
import { Input, Modal, Form, Table, Select, Alert, Icon, message, Spin, Tabs } from 'antd';
...
const ImgObj = {
  winPrize:`${activityTemplateObj}guaguaka/image.png`,
}
...
class PrizeTable extends PureComponent {
  ...
  getValues = () => {
    ...
    return { prizes: list, deleteIds, ...taskGroup, ...wxShareData }
  }
  ...
  render() {
    ...
    const { newAllPrizeList, visible, prizeState, prizeCurrent = {}, useInventory, list, popupValue, currTab } = this.state;
    const modalFooter = {
            ...
            >
              <Spin spinning={loading}>
                <Form className={styles.formHeight} onSubmit={this.prizeHandleSubmit}>
                  <FormItem label='奖项' {...this.formLayout}>
                  ...
                  <div style={{ display:'flex', padding:'0px 0px 20px 120px', alignItems:'center' }}>
                    <div style={{  flexGrow:100, alignItems:'center', position:'relative'  }}>
                      <div style={{ position:'absolute', top:10,  color:'#333' }} className={styles.edit_form_pre}>
                        <span style={{ color:'red', marginRight:5 }}>*</span>奖项图片:
                      </div>
                  ...
                  <FormItem label='弹窗文案' {...this.formLayout}>
                    {getFieldDecorator( 'popupText', {
                      rules: [{ message: `${formatMessage( { id: 'form.input' } )}弹窗文案` }],
                  ...
                  <FormItem
                    label='概率'
                    {...this.formLayout}
                    extra={<span style={{ color: '#D1261B', fontSize: 12 }}>*所有抽奖情况总概率相加需等于100%</span>}
                  >

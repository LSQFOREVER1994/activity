...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'scratvh_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addScratchCard' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityScratchCardUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityScratchCardUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='刮刮卡预览'
                frameBorder={0}
                src={`${serviceObj.activityScratchCardUrl}?preview=true`}

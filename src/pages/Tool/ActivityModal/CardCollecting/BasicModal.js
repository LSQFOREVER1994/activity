...
class BasicModal extends PureComponent {
  ...
  basicHandleSubmit = () => {
    ...
    form.validateFields( ( err, fieldsValue ) => {
      ...
      if ( moment( params.cardEndTime ).millisecond( Number ) > moment( params.lotteryStartTime ).millisecond( Number ) ) {
  ...
  dealData = ( fieldsValue ) => {
    ...
    params.cardStartTime = ( rangeTime && rangeTime.length ) ? moment( rangeTime[0] ).format( 'YYYY-MM-DD HH:mm:ss' ) : '';
    params.cardEndTime = ( rangeTime && rangeTime.length ) ? moment( rangeTime[1] ).format( 'YYYY-MM-DD HH:mm:ss' ) : '';
    ...
    params.endTime = ( lotteryTime && lotteryTime.length ) ? moment( lotteryTime[1] ).format( 'YYYY-MM-DD HH:mm:ss' ) : ( ( rangeTime && rangeTime.length ) ? moment( rangeTime[1] ).format( 'YYYY-MM-DD HH:mm:ss' ) : '' );
        ...
        <Form layout='horizontal' className={styles.formHeight} onSubmit={this.basicHandleSubmit}>
          ...
          >
            {getFieldDecorator( 'rangeTime', {
              ...
              initialValue: data.startTime ? [moment( data.cardStartTime, 'YYYY-MM-DD HH:mm:ss' ), moment( data.cardEndTime, 'YYYY-MM-DD HH:mm:ss' )] : [],
          ...
          >
            {getFieldDecorator( 'lotteryTime', {
              rules: [{ required: false, message: `${formatMessage( { id: 'form.input' } )}活动时间` }],
              initialValue: data.lotteryStartTime ? [moment( data.lotteryStartTime, 'YYYY-MM-DD HH:mm:ss' ), moment( data.lotteryEndTime, 'YYYY-MM-DD HH:mm:ss' )] : [],
            } )( <RangePicker showTime format="YYYY-MM-DD HH:mm:ss" /> )}
          </FormItem>
          <FormItem
            label='活动状态'

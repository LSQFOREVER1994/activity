...
class ListModal extends PureComponent {
  ...
  constructor(props) {
    let currentId = props.location.query.id || sessionStorage.getItem('fundCollision_edit_id') || ''
    if (props.location.pathname === '/oldActivity/activityModal/addFundCollision') {
      currentId = '';
    }
    super(props);
    this.state = {
  ...
  getEditData = (id) => {
    ...
    if (id) {
      dispatch({
        type: 'fundCollision/getFundCollision',
        payload: {
          id,
        },
        callFunc: (result) => {
  ...
  // 提交
  handleSubmit = () => {
    const { activeData, currentId } = this.state;
    !currentId ? this.onAddFundCollision() : this.onEditAddFundCollision();
  }
  ...
  onAddFundCollision = () => {
    ...
    if (basiceData && styleData && listData) {
      ...
      if (!data.prizes.length) {
        message.error("奖品设置不能为空");
        return;
      }
      dispatch({
        type: 'fundCollision/addFundCollision',
        payload: {
          params: data,
          callFunc: (res) => {
  ...
  onEditAddFundCollision = () => {
    ...
    if (basiceData && styleData && listData) {
      ...
      dispatch({
        type: 'fundCollision/editFundCollision',
        payload: {
          params: data,
          callFunc: (res) => {
            const currentId = res && res.id ? res.id : this.state.currentId;
            that.setState({ loading: false, currentId, visible: true, currentId })
          }
        },
      })
    }
  ...
  openActive = () => {
    ...
    window.open(`${serviceObj.fundCollisionUrl}?id=${currentId}`, '_blank', strWindowFeatures)
  }
  ...
  clickCopy = (e) => {
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy(`${serviceObj.fundCollisionUrl}?id=${currentId}`)
    if (tag) {
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='对对碰预览'
                frameBorder={0}
                src={`${serviceObj.fundCollisionUrl}?preview=true`}
                // src="http://lzydev.test.jiniutech.cn:8080/#/?preview=true"

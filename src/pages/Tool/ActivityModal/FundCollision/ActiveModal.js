...
import FundSetting from './PrizeCom/FundSetting'
...
class ActiveModal extends PureComponent {
  ...
  getValues = () => {
    ...
    const fundData = this.fundRef.getValues();
    return {
      ...prizeData,
      ...shareData,
      ...fundData,
  ...
  getHandleValues = () => {
    ...
    const fundData = this.fundRef.getData();
    if (prizeData) {
      return {
        ...prizeData,
        ...shareData,
  ...
  render() {
    ...
    const tabList = [
      {
        name: <div className={styles.edit_acitve_tab}>基金设置</div>,
        content: data &&
          <FundSetting
            data={data}
            onRef={(fund) => { this.fundRef = fund }}
            onPreview={onPreview}
          />
      },
      {

...
@connect(({ kLineArena }) => ({
  loading: kLineArena.loading,
}))
@Form.create()
class NoPrizeTable extends PureComponent {
  ...
  constructor(props) {
    super(props);
    this.state = {
      list: (props.drawLotsPrizeList && props.drawLotsPrizeList.length) ? props.drawLotsPrizeList.map(item => ({ ...item, rowKey: item.id })) : [],
      deleteIds: [0]
    };
  }
  ...
  getData = () => {
    ...
    return { drawLotsPrizeList: list, deleteIds }
  }
                ...
                <Form className={styles.formHeight} onSubmit={this.noPrizeHandleSubmit}>
                  ...
                  <FormItem label='签语' {...this.formLayout}>
                    {getFieldDecorator('explain', {
                      ...
                      initialValue: prizeCurrent.explain,
                    })(<TextArea
                      rows={2}
                      placeholder="请输入解签语"
                  ...
                  <FormItem
                    label='概率'
                    {...this.formLayout}
                    extra={<span style={{ color: '#D1261B', fontSize: 11 }}>*所有奖品情况概率之和需为100%</span>}
                  >

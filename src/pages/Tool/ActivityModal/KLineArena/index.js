...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'arenas_edit_id' ) || ''
    if ( props.location.pathname === '/oldActivity/activityModal/addKLineArena' ) {
      currentId = '';
    }
    super( props );
    this.state = {
  ...
  handleSubmit = () => {
    ...
    if( basiceData && styleData && listData ){
      ...
      console.log(data);
      if ( !data.lotteryRelative && !data.prizes.length ) {
        this.setState( {
          loading: false
        } )
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityKLineArenaUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy = ( e ) => {
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityKLineArenaUrl}?id=${currentId}` )
    if ( tag ) {
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='角斗场预览'
                frameBorder={0}
                src={`${serviceObj.activityKLineArenaUrl}?preview=true`}
                // src="http://192.168.2.20:8081/#/?preview=true"

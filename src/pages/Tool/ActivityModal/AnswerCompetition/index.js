...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'eatChickenAnswer_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addEatChickenAnswer' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  onPreview = () => {
    ...
    if ( basicRef && styleRef && activeRef ) {
      ...
      const data = Object.assign(
        ...
        styleData,
        listData,
       { answerTime:basiceData.answerTime||10 },
        { type: 'ANSWER_COMPETITION' }
      )
  ...
  handleSubmit = () => {
    ...
    if( listData && basiceData && styleData ){
      ...
      if( !listData.prizeData.lotteryRelative ) {
        if( !listData.prizeData.prizes ) {
          this.setState( { loading: false } )
          return message.error( '请配置奖品' )
        }
        if( listData.prizeData.prizes.length === 0 ) {
          this.setState( { loading: false } )
          return message.error( '请配置奖品' )
        }
      }
        ...
        callFunc:( packageId )=>{
          if( packageId ){
            ...
            delete activeData.nextState
              const data = Object.assign( activeData, basiceData, styleData, ruesltData, prizeData, wxShareData, { packageId }, { id:this.state.currentId }, { type: 'ANSWER_COMPETITION' }, { resultType:true,  answerTime:basiceData.answerTime||10  } )
              const newObj=data
              let prizeList=[]
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.answerCompetitionUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.answerCompetitionUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='吃鸡答题预览'
                frameBorder={0}
                src={`${serviceObj.answerCompetitionUrl}?preview=true`}

...
import { Input, Form, Alert, DatePicker, Radio, Row, Col, message, InputNumber } from 'antd';
...
class BasicModal extends PureComponent {
  ...
  constructor( props ) {
    ...
    this.state = {
      ...
      isShowAttendCount:props.data.isShowAttendCount === undefined ? true : props.data.isShowAttendCount,
    }
  }
  ...
  showAttendCountChange = ( e )=>{
    this.setState( { isShowAttendCount:e.target.value }, ()=>this.onChangePreview() )
  }
  ...
  render() {
    ...
    const { data, nameValue, isShowAttendCount } = this.state;
    return (
      <div>
        <Form layout='horizontal' className={styles.formHeight} onSubmit={this.basicHandleSubmit}>
          ...
          <FormItem label='参与次数展示' {...this.formLayout}>
            {getFieldDecorator( 'isShowAttendCount', {
              rules: [{ required: true, } ],
              initialValue: data.isShowAttendCount || isShowAttendCount
            } )(
              <Radio.Group onChange={this.showAttendCountChange}>
          ...
          <FormItem label='每题答题时间' {...this.formLayout}>
            {getFieldDecorator( 'answerTime', {
              ...
              initialValue: data.answerTime ||10,
            } )( <Input
              onChange={this.onChangePreview}
              placeholder='可设置每题作答时间，不填则默认10s'
              precision={0}
              min={0}
              addonAfter='秒'
              type='number'

...
const { Step } = Steps;
const orderBgTop = require( '../../../../assets/order_top.png' );
const orderBgBtn = require( '../../../../assets/order_btm.png' );
...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'placeOrder_edit_id' ) || ''
    if ( props.location.pathname === '/oldActivity/activityModal/addOrderCourtesy' ) currentId = ''
    super( props );
    this.state = {
      visible: false,
      currentStep: 0,
  ...
  onPreview = () => {
    ...
    if ( activeRef ) {
      ...
      const prizeImgSrc = listData && listData.prizeList && listData.prizeList[0] && listData.prizeList[0].image ? listData.prizeList[0].image : "";
      this.setState( { prizeImgSrc } );
    }
  ...
  handleSubmit = () => {
    ...
    if ( basiceData && listData ) {
      const data = Object.assign( activeData, basiceData, { ...listData } );
  ...
  openActive = () => {
    ...
    window.open( `https://m.jiniutech.com/s-mall/#/?id=${currentId}`, '_blank', strWindowFeatures )
  }

  // 拷贝
  clickCopy = ( e ) => {
    // https://m.jiniutech.com/s-mall/#/
    // http://s-mall.test.jiniutech.cn/v2/#/
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `https://m.jiniutech.com/s-mall/#/?id=${currentId}` )
    if ( tag ) {
  ...
  render() {
    const {
      currentStep, activeData, loading, canChangeStep, currentId, visible, prizeImgSrc
    } = this.state;

    // TODO:
    // const activeBgImg = activeData && activeData.prizeList && activeData.prizeList.length ? activeData.prizeList[0].image : "";
    const steps = [
      {
        title: '基础设置',
        content: activeData &&
        ...
        >
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <div className={styles.order_courtesy}>
                <div>
                  <img style={{ width: "100%", height: "100%" }} alt="" src={orderBgTop} />
                </div>
                <div className={styles.order_courtesy_prizeImg}>
                  <img style={{ width: "100%" }} alt="" src={orderBgBtn} />
                  {prizeImgSrc &&
                    <div className={styles.prizeImgSrc}>
                      <img src={prizeImgSrc} alt="" />
                    </div>
                  }
                </div>
              </div>

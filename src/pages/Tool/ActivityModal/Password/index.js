...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'password_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addPassword' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  handleSubmit = (  ) => {
    ...
    const listData = this.activeRef.getValues();
    const list = listData.prizes;
    let newList = [];
    list.forEach( item => newList.push( item.description ) );
    newList = Array.from( new Set( newList ) );
    if( ( newList && newList.length ) !== ( list && list.length ) ){
      message.error( '同一活动内口令不能重复' )
      return false
    }
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityPasswordUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityPasswordUrl}?id=${currentId}` )
    if( tag ){
    ...
    return (
      <GridContent className={styles.collect_edit_grid} style={{ background:'#fff' }}>
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='口令活动预览'
                frameBorder={0}
                src={`${serviceObj.activityPasswordUrl}?preview=true`}
        ...
        <Modal
          ...
          maskClosable={false}
        >
          <h3>{( this.props.location.pathname === '/oldActivity/activityModal/addPassword' && currentId ) ? '保存活动成功' : '创建活动成功'}</h3>
          <p>后续可在活动面板进行编辑和查看～</p>

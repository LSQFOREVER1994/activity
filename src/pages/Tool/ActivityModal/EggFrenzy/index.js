...
@Form.create()
class EggFrenzy extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'egg_edit_id' ) || ''
    if ( props.location.pathname === '/oldActivity/activityModal/addEggFrenzy' ) currentId = ''
    super( props );
    this.state = {
      visible: false,
      currentStep: 0,
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityEggsUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy = ( e ) => {
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityEggsUrl}?id=${currentId}` )
    if ( tag ) {
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='砸金蛋预览'
                frameBorder={0}
                src={`${serviceObj.activityEggsUrl}?preview=true`}
                // src="http://lzydev.test.jiniutech.cn:7456/?preview=true"

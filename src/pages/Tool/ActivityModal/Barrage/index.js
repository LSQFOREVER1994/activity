...
@Form.create()
class Barrage extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'barrage_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addBarrage' ) currentId = null
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  handleSubmit = () => {
    ...
    if( isEmptyValue ){
     message.error( "弹幕内容不能为空!" );
     return
    }
    this.submitActivityData( Parameters )

    // if ( currentId ){
    //   // 编辑
    //   this.editBulletScreen( Parameters )
    // }else{
    //   // 新增
    //   this.addBulletScreen( Parameters )
    // }
  }

  submitActivityData=( data )=>{
    const { dispatch }=this.props;
    dispatch( {
      ...
      payload: {
        ...
        callFunc: ( res ) => {
          if( res ){
            const currentId = res && res.id ? res.id : '';
  ...
  // // 新增活动
  // addBulletScreen=( Parameters )=>{
  //   const { dispatch } = this.props;
  //   dispatch( {
  //     type: 'bulletScreen/addBulletScreen',
  //     payload: {
  //       params:Parameters,
  //       callFunc: ( res ) => {
  ...
  // // 活动编辑
  // editBulletScreen=( Parameters )=>{
  //   const { dispatch } = this.props;
  //   dispatch( {
  //     type: 'bulletScreen/editBulletScreen',
  //     payload: {
  //       params:Parameters,
  //       callFunc: ( res ) => {
  //         const currentId = res && res.id ? res.id : this.state.currentId;
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.barrageUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.barrageUrl}?id=${currentId}` )
    if( tag ){
        ...
        >
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                className={styles.collect_iframe_barrage}
                scrolling='auto'
                title='弹幕活动预览'
                frameBorder={0}
                src={`${serviceObj.barrageUrl}?preview=true`}

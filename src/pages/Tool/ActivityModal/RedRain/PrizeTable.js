...
class PrizeCom extends PureComponent {
  ...
  constructor( props ) {
    ...
    this.state = {
      ...
      prizeState: true,
      hintState: false,
      lotteryNum: props.data && props.data.atLeast ? props.data.atLeast : ''
    };
  }
  ...
  //  提交数据
  getData = () => {
    const { list, deleteIds, lotteryNum } = this.state;
    //  处理抢到红包个数
    if ( lotteryNum === '' ) {
      this.setState( {
        hintState: true,
    ...
    return { prizes: list, deleteIds, atLeast: lotteryNum }
  }
  ...
  lotteryChang = ( e ) => {
    if ( e.target.value === '' ) {
      ...
      this.setState( {
        lotteryNum: e.target.value < 1 ? 1 : e.target.value,
        hintState: false
      } )
    }
  }
  ...
  render() {
    ...
    const { visible, prizeCurrent = {}, useInventory, list, popupValue, newAllPrizeList, prizeState, hintState, lotteryNum } = this.state;
      ...
      <GridContent>
        <div className={styles.red_rain_prize}>
          <div className={styles.lottery_prize}>
            <p className={styles.lottery_prize_name}><span style={{ color: 'red' }}>*</span>抽奖需至少抢到红包：</p>
            <Input
              value={lotteryNum}
              type='number'
              min={1}
              max={222}
              onChange={this.lotteryChang}
              className={styles.lottery_prize_input}
              placeholder="请输入抽奖需至少抢到红包个数"
              addonAfter='个'
            />
          </div>
          {
            hintState && <div className={styles.mistake_hint}>请输入抽奖需至少抢到红包个数，抢到红包个数最小值为1，最大值为999999999</div>
          }
          <Alert

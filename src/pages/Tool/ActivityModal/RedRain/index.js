...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'redRain_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addRedRain' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityRedRainUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityRedRainUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='红包雨预览'
                frameBorder={0}
                src={`${serviceObj.activityRedRainUrl}?preview=true`}

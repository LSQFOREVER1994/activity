...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'draw_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addDraw' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
        ...
        payload: {
          ...
          callFunc: ( res ) => {
            if( res ){
              ...
              that.setState( { currentId, visible:true } )
            }
          },
        },
      } )
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityDrawUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityDrawUrl}?id=${currentId}` )
    if( tag ){
  ...
  render() {
    const {
      currentStep, activeData, canChangeStep, currentId, visible
    } = this.state;

    const { activityLoading } = this.props;
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='抽签预览'
                frameBorder={0}
                src={`${serviceObj.activityDrawUrl}?preview=true`}
                // src="http://10.10.66.6:8080/#/?preview=true"
                ref={( Ifram ) => { this.iframe = Ifram}}
                id='myframe'
                onLoad={this.onPreview()}
              />
            </div>

...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'bigwheel_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addBigWheel' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  handleSubmit = () => {
    ...
    if ( basiceData && styleData && listData ){
      ...
      if( data.prizes.length !== 8 ) {
        this.setState( {
          loading: false
        } )
        return message.error( '至少配置八个奖品' )
      }
      dispatch( {
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.activityBigWheelUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.activityBigWheelUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='大转盘预览'
                frameBorder={0}
                src={`${serviceObj.activityBigWheelUrl}?preview=true`}

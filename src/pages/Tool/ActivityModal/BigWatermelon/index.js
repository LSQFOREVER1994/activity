...
import ActiveModal from './Activity/Activity';
...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'bigWatermelon_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addBigWatermelonGame' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
    ...
    if( id ){
      dispatch( {
        ...
        callFunc: ( result ) => {
          ...
          console.log( result );
          console.log( '----------接口请求回来的数据---------' );
          this.setState( {
            activeData
          }, ()=>{
            console.log( this.state.activeData );
            console.log( '----------设置后请求回来的数据---------' );
              setTimeout( () => {
                this.onPreview()
              }, 1000 )
          } )
  ...
  onPreview = () => {
    ...
    if ( basicRef && styleRef && activeRef ) {
      ...
      const jnApiResult = Object.assign( activeData,
        basiceData,
        styleData,
        { ...listData, },
      )
      if( this.iframe && this.iframe.contentWindow ) {
        delete jnApiResult.nextState;
        // console.log( jnApiResult );
        // window.jnApiResult = jnApiResult;
        this.iframe.contentWindow.postMessage( { jnApiResult }, '*' );
      }
    }
  }
  ...
  handleSubmit = () => {
    ...
    const listData = this.activeRef.getData();
    const { activeData } = this.state;
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.bigWatermelonUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.bigWatermelonUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='合成大西瓜预览'
                frameBorder={0}
                src={`${serviceObj.bigWatermelonUrl}?preview=true`}
                // src="http://192.168.12.14:8080?preview=true"

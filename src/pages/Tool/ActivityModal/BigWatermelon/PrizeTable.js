...
import { Input, Modal, Form, Table, Select, Alert, Icon, message, Tabs, Spin, Radio } from 'antd';
...
class PrizeCom extends PureComponent {
  ...
  constructor( props ) {
    ...
    this.state = {
      ...
      // 抽奖链接
      lotteryRelative:props.data.lotteryRelative === undefined ? 1 : props.data.lotteryRelative,
      // 奖品发放规则
      prizeSendMode: 'rank',
    };
  }
  ...
  changePrizeSendMode =( e )=>{
    ...
    //   } )
    // }
    this.setState( { prizeSendMode:e.target.value } )
  }

  render() {
    const { loading, form: { getFieldDecorator }, data } = this.props;
    const {
            ...
            <Alert
              ...
              message={(
                <div style={{ fontSize: 12, width: '100%', display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <span onClick={() => { window.open( `${window.location.origin}/tool/prizeManage` )}} style={{ color: '#1890FF', cursor:'pointer' }}>奖品管理</span>
                </div> )}
              banner
            />

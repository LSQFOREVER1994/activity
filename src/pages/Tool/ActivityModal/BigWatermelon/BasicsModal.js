...
import { Input, Form, Alert, DatePicker, Radio, Row, Col, message, Checkbox } from 'antd';
...
class BasicModal extends PureComponent {
  ...
  getValues = () =>{
    ...
    // 对排行榜选项排序
    let rankOptions = ['DAY', 'WEEK', 'MONTH', 'ALL'];
    rankOptions =  rankOptions.filter( item => {
      return data.rankOptions && data.rankOptions.indexOf( item ) !== -1
    } );
    if ( data.rankOptions ) {
    ...
    return (
      <div>
        <Form layout='horizontal' className={styles.formHeight} onSubmit={this.basicHandleSubmit}>
          ...
          <FormItem label='排行榜选项' {...this.formLayout}>
            {getFieldDecorator( 'rankOptions', {
              initialValue: data.rankOptions || []
            } )(
              <Checkbox.Group style={{ width: '100%' }} onChange={this.onPreview}>
                <Checkbox value="DAY">日榜</Checkbox>
                <Checkbox value="WEEK">周榜</Checkbox>
                <Checkbox value="MONTH">月榜</Checkbox>
                <Checkbox value="ALL">总榜</Checkbox>
                <span style={{ fontSize: '12px', color: '#d9d9d9', marginLeft: '4px' }}>*排行榜可多选，如均未勾选则默认无排行榜功能</span>
              </Checkbox.Group>
              )}
          </FormItem>

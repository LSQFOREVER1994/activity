...
class ListModal extends PureComponent {
  ...
  constructor( props ) {
    let currentId = props.location.query.id || sessionStorage.getItem( 'prizeclaw_edit_id' ) ||  ''
    if ( props.location.pathname === '/oldActivity/activityModal/addPrizClaw' ) currentId = ''
    super( props );
    this.state = {
      visible:false,
      currentStep: 0,
  ...
  handleSubmit = () => {
    ...
    if ( basiceData && styleData && listData ){
      ...
      // 判断当上传奖品图片少则取消上传
      if( data.prizeImgList.length === 0 ) {
        this.setState( { loading: false } )
        return message.error( '请至少上传一张图片' )
      }
  ...
  openActive = () => {
    ...
    window.open( `${serviceObj.prizeClawUrl}?id=${currentId}`, '_blank', strWindowFeatures )
  }
  ...
  clickCopy=( e )=>{
    e.stopPropagation();
    const { currentId } = this.state;
    const tag = copy( `${serviceObj.prizeClawUrl}?id=${currentId}` )
    if( tag ){
          ...
          <div className={styles.collect_edit}>
            <div className={styles.collect_edit_left}>
              ...
              <iframe
                ...
                title='抓娃娃预览'
                frameBorder={0}
                src={`${serviceObj.prizeClawUrl}?preview=true`}

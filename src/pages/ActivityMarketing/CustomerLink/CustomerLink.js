 ...
 * @LastEditTime: 2024-08-23 16:48:04
...
import styles from './customerLink.less';
...
const timeRanges = [
 ...
 function CustomerLink( props ) {
  const { closeUserActionPage, dispatch, activityId, form, customerData, customerList, selectCustomerList, loading } = props
  ...
  const getActivityCustomerlList = () => {
    dispatch( {
      type:'customerDetail/getActivityCustomerlList',
      payload:{
        query:{
          activityId
        },
  ...
  const getActivityCustomerData = () => {
    ...
    dispatch( {
      type:'customerDetail/getActivityCustomerData',
      payload:{
        query:{
          appId:activityId,
          beginTime,
  ...
  const getSelectChannelList = () => {
    dispatch( {
      type:'customerDetail/activityAllCustomer',
      payload:{
        query:{},
        callFunc:()=>{
  ...
  const getAddNewChannel = ( params ) => {
    dispatch( {
      type:'customerDetail/addActivityCustomer',
      payload:{
        query:params,
        callFunc:()=>{
          getActivityCustomerlList()
  ...
  const updateCustomerStatus = ( params ) => {
    dispatch( {
      type:'customerDetail/updateCustomerStatus',
      payload:{
        query:params,
        callFunc:()=>{
          getActivityCustomerlList()
        ...
        <Form>
          <FormItem {...formItemLayout} label="客户经理名称">
            {getFieldDecorator( 'managerId', {
              rules: [{ required: true, message: '请选择客户经理名称' }],
            } )( 
              <Select
                placeholder="请选择客户经理名称"
                showSearch
              ...
              >
                {
                 selectCustomerList?.length && selectCustomerList.map( ( item )=>(
        ...
        return (
          <div>
            ...
            <span 
              ...
              onClick={()=>{ generateAndDownloadQRCode( record.link )}}
            >
              下载二维码
            </span>
            <Popconfirm
              ...
              onConfirm={()=>{ updateCustomerStatus( { id: record.id, status: !record.status } )}}
              okText="是"
              cancelText="否"
            >
              <span 
  ...
  return (
    <GridContent>
      <Breadcrumb style={{ marginBottom: 20 }}>
        ...
        <Breadcrumb.Item>客户经理数据分析</Breadcrumb.Item>
      </Breadcrumb>
      <Card
        bordered={false}
        title='客户经理链接'
      ...
      >
        <div style={{ textAlign:'right', marginBottom:'20px' }}>
          <Button type='primary' onClick={getSelectChannelList}>新增客户经理链接</Button>
        </div>
        <Table 
...
export default connect( ( state )=>( {
  ...
  customerList:state.customerDetail.customerList,
  selectCustomerList:state.customerDetail.selectCustomerList,
  customerData:state.customerDetail.customerData,
} ) )( Form.create()( CustomerLink )  ) ;
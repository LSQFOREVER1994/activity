...
class ReceiveGoldData extends PureComponent {
  constructor( props ) {
    super( props )
    this.state = {
      pageNum: 1,
  ...
  getUserRecord = () => {
    ...
    dispatch( {
      type: 'receiveGold/getUserRecord',
      payload: {
        activityId,
        userId,
  ...
  exportRecord = () =>{
    ...
    let ajaxUrl = `receive-gold/records/export`
    if ( paramsArray && paramsArray.length > 0 ) {
      const paramStr = paramsArray.join( '&' )
      ajaxUrl = `receive-gold/records/export?${paramStr}`
    }
    this.setState( {
      isExPLoading: true
    }, () => {
  ...
  render() {
    ...
    const exportConfig = {
      type: 'activityService',
      ajaxUrl: `receive-gold/users/export`,
      xlsxName: '接元宝游戏数据.xlsx',
    ...
    return (
      <GridContent>
        <Breadcrumb style={{ marginBottom: 20 }}>
          ...
          <Breadcrumb.Item>接元宝游戏数据</Breadcrumb.Item>
        </Breadcrumb>
        <Row gutter={24}>
          <Col span={8}>
            <Card
...
export default ReceiveGoldData;

...
class DollMachineData extends PureComponent {
  constructor( props ) {
    super( props )
    this.state = {
      pageNum: 1,
  ...
  getUserRecord = () => {
    ...
    dispatch( {
      type: 'dollMachine/getUserRecord',
      payload: {
        activityId,
        userId,
  ...
  exportRecord = () =>{
    ...
    let ajaxUrl = `element/doll-machine/records/export`
    if ( paramsArray && paramsArray.length > 0 ) {
      const paramStr = paramsArray.join( '&' )
      ajaxUrl = `element/doll-machine/records/export?${paramStr}`
    }
    this.setState( {
      isExPLoading: true
    }, () => {
  ...
  renderModal = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: 'create_time',
        sorter: true,
        sortOrder: userSortedInfo.columnKey === 'create_time' && userSortedInfo.order,
  ...
  render() {
    ...
    const exportConfig = {
      type: 'activityService',
      ajaxUrl: `element/doll-machine/users/export`,
      xlsxName: '抓娃娃游戏数据.xlsx',
    ...
    return (
      <GridContent>
        <Breadcrumb style={{ marginBottom: 20 }}>
          ...
          <Breadcrumb.Item>抓娃娃游戏数据</Breadcrumb.Item>
        </Breadcrumb>
        <Row gutter={24}>
          <Col span={8}>
            <Card
...
export default DollMachineData;

...
import { Table, Button, Form, Radio, DatePicker, Tooltip, Icon, Drawer } from 'antd';
...
class Event extends PureComponent {
  ...
  jumpEventDetail = ( e, name, type ) => {
    ...
    const eventInfo = { name, appId, type }
    this.setState( {
      eventInfo,
  ...
  render() {
    const { from, to, timeType, eventInfo } = this.state;
    const { loading, eventSum } = this.props;
    const columns = [
      {
        title: <span>事件名</span>,
        dataIndex: 'name',
      ...
      },
      {
        title: () => (
          <span>
            总用户数
            <Tooltip placement="top" title='日期范围内事件触发的总用户数'>
              <Icon style={{ fontSize: '15px', marginLeft: '10px' }} type="question-circle" />
            </Tooltip>
          </span>
        ),
      ...
      },
      {
        ...
        dataIndex: 'usersClick',
        render: ( _, record ) => <span>{record.users && Number( record.triggers / record.users ).toFixed( 2 )}</span>,
      },
      {
        title: '详情',
        render: ( text, record ) => (
    ...
    return (
      <div style={{ backgroundColor: 'white', marginTop: 20 }}>
        ...
        />
        <Drawer
          title={eventInfo && eventInfo.name}
          placement="right"
          closable
          onClose={this.onCloseEventInfo}

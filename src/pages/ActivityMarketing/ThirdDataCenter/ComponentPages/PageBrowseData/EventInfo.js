...
class EventInfo extends PureComponent {
  ...
  initEventChartsData = () => {
    ...
    const types = ['getEventDaily', 'getEventValueSum'];
    const arr = types.map( i => {
      return dispatch( {
        type: `statistics/${i}`,
        payload: { from, to, appId, name }
      } )
    } )
    Promise.all( arr )
  }
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: <span>占比</span>,
        dataIndex: 'triggersPercent',
        render: ( item, record ) => <span>{Number( record.triggers / sumTriggers * 100 ).toFixed( 2 )}%</span>,
      },
    ];
    ...
    return (
      <Spin spinning={loading}>
        ...
        <div style={{ backgroundColor: 'white', padding: '0 15px' }}>
          ...
          {showDetail && <Table rowKey="date" size="middle" dataSource={eventDaily} columns={this.tableColums} pagination={{ defaultPageSize: 20 }} />}
        </div>

...
class PrizeTable extends PureComponent {
  ...
  componentWillReceiveProps( nextProps ) {
    const { elementShowType, tags=[] } = nextProps.eleObj;
    if( elementShowType === 'HOMEPAGE' ){
      this.setState( {
        tagActive:tags.length ? tags[0].name : ''
      } )
    }
  }
  ...
  initPeizeData = () => {
    const { eleObj = {} } = this.props;
    const { prizes, elementShowType, tags=[] }= eleObj;
    if( elementShowType === 'HOMEPAGE' ){
      this.setState( {
        tagActive:tags.length ? tags[0].name : ''
      } )
  ...
  // 奖品表格
  renderPrizeTable = () => {
    const { columns, tagActive } = this.state
    const { eleObj } = this.props;
    const { elementShowType, prizes } = eleObj || {};
    let listData = [];
    if( elementShowType === 'HOMEPAGE' ){
      listData = prizes.filter( o=> o.tagName === tagActive )
    }else{
      listData = prizes || []
    }
    const components = {
  ...
  renderTabs = () => {
    ...
    const { eleObj } = this.props;
    const { elementShowType, tags=[] }= eleObj
    if ( elementShowType !== 'HOMEPAGE' ) return null;
      ...
      <div className={styles.tagsBox}>
        {
          tags.length > 0 && tags.map( ( item, index )=>(
            <div
              onClick={()=>{this.tabsHandler( item, index )}}
              className={`${styles.tags} ${tagActive===item.name ? styles.active : ''}`}
              key={index}
            >
              {item.name}
            </div>
  ...
  render() {
    const { prizeModalVisible, editPositionKey, tagActive } = this.state
    const { domData = {}, changeDomData, eleObj = {} } = this.props;
    return (
      <div>

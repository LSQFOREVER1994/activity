...
const keyVerification = [ 'relationPrizeType', 'relationPrizeId', 'name', 'expireType']
...
@Form.create()
class EditCardPrizeModal extends PureComponent {
  formLayout = {
    labelCol: { span: 4 },
    wrapperCol: { span: 16 },
  };
  ...
  onEdit = () => {
    if ( !this.onPrizeModalVerification(  ) ) return
    const { saveObj } = this.state
    const { domData = {}, changeDomData, eleObj = {}, onChangeVisible, editPositionKey } = this.props;
    // 整理数据
    const prizeList = [...eleObj.prizes]
  ...
  onPrizeModalVerification = () => {
    ...
    if ( saveObj.inventory > this.getUnIssuedCnt() ) {
  ...
  onItemChange = ( e, type ) => { // 塞入奖品项
    ...
    if( type === 'rankFrom'  ) {
      saveObj.inventory = ( saveObj.rankTo - ( e ) ) + 1
    } else if ( type === 'rankTo' ) {
      saveObj.inventory = ( e - saveObj.rankFrom ) + 1
    }
    ...
    if( type === 'relationPrizeId' ) {
      saveObj.inventory = ( saveObj.rankTo - saveObj.rankFrom ) + 1
      ...
      <div>
        ...
        >
          <div>
            ...
            <FormItem
              label={<span className={styles.labelText}><span>*</span>名次</span>}
              {...this.formLayout}
            >
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <InputNumber
                  value={saveObj.rankFrom ? saveObj.rankFrom : 0}
                  placeholder="请输入首位名次"
                  min={1}
                  style={{ width: '50%' }}
                  ...
                  precision={0}
                  onChange={( e ) => this.onItemChange( e, 'rankFrom' )}
                />
                <span style={{ margin: '0 10px' }}>至</span>
                <InputNumber
                  value={saveObj.rankTo ? saveObj.rankTo : 1}
                  placeholder="请输入末位名次"
                  min={( saveObj.rankFrom || 1 )}
                  style={{ width: '50%' }}
                  formatter={limitDecimals}
                  parser={limitDecimals}
                  precision={0}
                  onChange={( e ) => this.onItemChange( e, 'rankTo' )}
                />
              </div>
            </FormItem>
            {
...
export default EditCardPrizeModal;

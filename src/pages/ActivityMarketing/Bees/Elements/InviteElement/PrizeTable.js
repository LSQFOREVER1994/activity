...
class PrizeTable extends PureComponent {
  ...
  onDeletePrizeItem = ( e, item ) => {
    const { configPrizeKey = 'prizes' } = this.props;
    e.stopPropagation();
  ...
  initPeizeData = () => {
    const { configPrizeKey = 'prizes' } = this.props;
    const { eleObj = {} } = this.props
    let newPrizes = eleObj[configPrizeKey] ? eleObj[configPrizeKey] : []
    if ( eleObj[configPrizeKey] && eleObj[configPrizeKey].length > 0 ) {
      newPrizes = eleObj[configPrizeKey].map( info => {
        return {
          ...info,
  ...
  onDeltet = ( probabilityNum ) => {
    const { eleObj = {}, configPrizeKey = 'prizes' } = this.props
    const { columns = [] } = this.state
    // 删除限制
    let prohibitNum = 1
    if ( eleObj[configPrizeKey] && eleObj[configPrizeKey].length > 0 ) {
      const obj = eleObj[configPrizeKey][0] || []
    ...
    const newList = eleObj[configPrizeKey].map( info => {
      const newInfo = info
  ...
  onAddProbability = () => {
    ...
    let newList = eleObj[configPrizeKey]
    if ( !eleObj[configPrizeKey] || ( eleObj[configPrizeKey] && !eleObj[configPrizeKey].length ) ) {
    ...
    if ( eleObj[configPrizeKey] && eleObj[configPrizeKey].length > 0 ) {
      ...
      // 更新列表数据
      newList = eleObj[configPrizeKey].map( info => {
        return {
          ...info,
  ...
  handleSave = ( key, row ) => {
    const { eleObj, configPrizeKey = 'prizes' } = this.props;
    const listData = eleObj[configPrizeKey] || []
  ...
  updatePrizeListData = ( list ) => {
    const { domData = {}, changeDomData, eleObj = {}, configPrizeKey = 'prizes' } = this.props;
    const elementsList = domData.elements ? domData.elements : []
    const newEleObj = Object.assign( eleObj, { [configPrizeKey]: [...list] } );
    // 替换对应项
  ...
  render() {
    ...
    const { domData = {}, changeDomData, eleObj = {}, title, configPrizeKey = 'prizes' } = this.props;
    return (
      <div>

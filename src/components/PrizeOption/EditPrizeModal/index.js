...
const needWeekMonthLimit = ['GRID_WHEEL', 'BIG_WHEEL', 'SCRATCH_CARD', 'MONOPOLY', 'RED_RAIN', 'SLOT_MACHINE', 'MYSTERY_BOX', 'SMASH_EGG', 'LUCK_DOG', 'AWARD', "CARD", "ACTIVITY_SIGN", "INVITE", 'TASK', 'ANSWER', 'EXCHANGE', 'COMPOSITE_GAME', 'RECEIVE_GOLD', 'FEED_UPGRADE', 'K_LINE_GAME', 'PUZZLE_GAME', 'DOLL_MACHINE', 'FLIP']
...
const EditPrizeModal = props => {
  ...
  const renderPrizeOptionItem = () => {
    ...
    return (
      <>
        {content}
        {noMore && prizeOptionList.length >= 1 ? '' :  ( 
  ...
  const renderPrizeOption = useCallback( () => {
    ...
    return (
      <>
        {
          rewardType === 'PRIZE' && (
            <FormItem label="发奖模式" {...formLayout} hidden>
  ...
  }, [awardMode, rewardType, prizeOptionList] );
